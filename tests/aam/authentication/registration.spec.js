...
async function clickNext(page) {
  const nextBtn = page.getByRole('button', { name: 'Next' }).filter({ has: page.locator(':visible') });
...
test.describe('Registration and Nurse Flow', () => {
  ...
  await page.getByRole('textbox', { name: /First Name/i }).fill(faker.person.firstName());
  await page.getByRole('textbox', { name: /Last Name/i }).fill(faker.person.lastName());
  ...
  const successMessage = await page.locator('div.message').textContent();
    const beneficiaryIdMatch = successMessage.match(/Beneficiary ID is : (\d+)/);
    beneficiaryId = beneficiaryIdMatch ? beneficiaryIdMatch[1] : null;

    expect(beneficiaryId).not.toBeNull();

        const __filename = fileURLToPath(import.meta.url);
    const __dirname = path.dirname(__filename);
    const filePath = path.resolve(__dirname, '../../../beneficiary.json');
    fs.writeFileSync(filePath, JSON.stringify({ beneficiaryId }));
